...
def load_dataset(img_nums, n_augmentation_per_image, batch_size=1):
    ...
    masks  = np.zeros(shape=((n_augmentation_per_image) * len(img_nums), 650, 650), dtype=np.int64)
...
def load_dataset_predict(input_dir, output_dir, instance_names, batch_size=1):
    ...
    masks = np.zeros(shape=(len(instance_names), 650, 650), dtype=np.int64)
    output_paths = []
    ...
    for inst in instance_names:
        try:
            ...
            before_path = os.path.join(input_dir, inst, "before.png")
            after_path = os.path.join(input_dir, inst, "after.png")
            mask_path = os.path.join(input_dir, inst, "mask.png")

            # Debug: Log file paths
            print(f"Checking files for instance {inst}:")
            ...
            if not os.path.exists(before_path) or not os.path.exists(after_path):
                print(f"WARNING: Missing required files for instance {inst}. Skipping...")
                continue

            # Opening
            img_b = open_image(before_path)
            img_a = open_image(after_path)
            img_m = np.zeros(shape=(650, 650))  # Default mask if not provided
            if os.path.exists(mask_path):
                img_m = open_image(mask_path)
            ...
            inputs[i] = input
            masks[i] = mask
            output_paths.append(os.path.join(output_dir, inst))
            i += 1
        except Exception as e:
            print(f"ERROR: Failed to process instance {inst}: {e}")

    inputs = inputs[:i]
    masks = masks[:i]

    if len(inputs) == 0 or len(masks) == 0:
        raise ValueError("No valid data found. Check input directory and instance names.")
...
def dataset_to_dataloader(inputs, masks):
    ...
    """
    if len(inputs) == 0 or len(masks) == 0:
        raise ValueError("Inputs or masks are empty. Ensure the dataset is correctly loaded.")
...
def grey_split(mask):
    ...
    """
    grey = np.zeros(shape=(mask.shape[0], mask.shape[1]), dtype=np.int32)  # or np.int64
    grey[...] = mask[..., 0]
    grey[grey != 0] = 1.0
    return grey
...
def plot_accuracy_histogram(predicted_masks, ground_truth_masks, filename="accuracy_histogram.png"):
    """
    Plots a histogram of accuracy metrics for predicted and ground truth masks.
    :param predicted_masks: list of predicted masks
    :param ground_truth_masks: list of ground truth masks
    :param filename: file path to save the histogram
    """
    accuracies = []
    ...
    for pred, gt in zip(predicted_masks, ground_truth_masks):
        ...
        accuracy = np.sum(pred_flat == gt_flat) / len(gt_flat)
        accuracies.append(accuracy)

    # Plot histogram
    plt.figure(figsize=(10, 6))
    plt.hist(accuracies, bins=10, color='blue', alpha=0.7, edgecolor='black')
